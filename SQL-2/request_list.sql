...
-- главная картинка:
SELECT c.caption, p.caption, pic.alt, pic.url
FROM Products AS p
JOIN Chapters AS c
    ON p.chapterId = c.id
...
-- описание
SELECT p.caption, pic.alt, pic.url,
    p.price, p.priceWithDiscount,
    p.priceWithPromoCode, p.description
FROM Products AS p
JOIN Pictures AS pic
    ON p.pictureId = pic.id
